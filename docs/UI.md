...
```
        0 ┌──────────────────────────────┐
          │  ステータス 1段目 (16px)      │   Gen / Day
       16 ├──────────────────────────────┤
          │  ステータス 2段目 (16px)      │   時間帯・ごきげん・満腹度
       32 ├──────────────────────────────┤
          │                              │
          │                              │
          ...
          │                              │
      110 ├──────────────────────────────┤
          │  メニューバー (18px)          │   アイコン5個、黒地に白
      128 └──────────────────────────────┘
```

キャラ領域は 128 × 78。ドット絵に十分な余裕があり、表情や動きで状態を表現できる。
...
- `Gen:3` = 第3世代、`Day:4` = ゲーム内4日目
- **時間帯** = あさ / ひる / ゆう / よる。ペットは 0 時に寝るので、
  `よる` が「その日の世話を済ませておく」合図になる
- `♥` = ごきげん（4段階）、`▮` = 満腹度（4段階）
- **メニューバーは黒地に白アイコン、選択中のセルだけ白地に黒**。
  バーが1つの帯として読め、カーソルがその「抜き」として目に入る
- **常にどれか1つが選択されている**
...
タイトルを「あっちむいて..」と省略しているのは、全角8文字が 1 行に入らないため。
2 行に折ると縦を 1 行分余計に食い、それに見合う情報が末尾の「ホイ」に無い。
