# 画面デザイン・遷移・ボタン割り当て（モック）

対象: 128 × 128 モノクロ OLED（1bpp）

> モック中の絵文字は**見た目の説明用**。実機は 1bpp なので、実際は自前のビットマップアイコンになる。

> **表示文字列について**: 以下のモックは日本語で書いてあるが、**実装は ASCII を使う**。
> `ja` フォントはイメージに入っているものの、`blitstr2` の `english_rules` は
> baosec 向けビルドで `zh` にしかフォールバックせず、`ja` を参照しない
> （`libs/blitstr2/src/style_macros.rs`）。`locales/lang-ja` を有効にしても直らない —
> `style_glyph()` はロケールを `"jp"` と比較するが、`locales` が返すのは `"ja"` で一致しない
> （`libs/blitstr2/src/lib.rs`）。どちらも `xous-core` の修正が必要なので、
> 公式のまま使う方針を優先して ASCII にしている。

---

## 1. 入力

`services/bao1x-hal-service/src/servers/keyboard.rs` の `map_keypress()` が、
物理入力を以下の文字にマップしている。

| キーコード | 物理入力 | 位置 |
|---|---|---|
| `←` | `KeyPress::Left` | **左ボタン** |
| `🔥` | `KeyPress::Center` | **中央ボタン** |
| `→` | `KeyPress::Right` | **右ボタン** |
| `↑` `↓` | `KeyPress::Up` / `Down` | ジョグ（別位置） |
| `∴` | `KeyPress::Select` | ジョグ押し込み（別位置） |

`🔥` は "Fire" の意。ソースのコメントによれば、キャリッジリターン（`\r`）が
シェルの行末に予約されているため、中央キーには専用の文字が割り当てられている。

`dc34-vault` でも `←` `→` `🔥` の3つを一組として扱う判定があり（`ux.rs:286`）、
この3つが横並びの3ボタンであることを裏付けている。

### 割り当て

**上下キー（`↑` `↓`）は使わない。** ゲーム内の操作は前面3ボタンで完結させる。

| ボタン | キーコード | 役割 |
|---|---|---|
| **左** | `←` | **選択** — カーソルを1つ進める（右端で左端へ循環） |
| **中央** | `🔥` | **決定** — 選択中の項目を実行 |
| **右** | `→` | **キャンセル** — 戻る / 選択解除 |
| ジョグ押し込み | `∴` | **ゲームを抜ける** — バッジのメニューへ |

`∴` だけは例外的に使う。当初はメイン画面で `→` を押したときに抜ける設計だったが、
**誤操作で頻繁に抜けてしまう**ため変更した。`∴` は前面3ボタンから離れた位置にあり、
公式アプリでもメニューキーとして使われているので、役割としても一貫する。

`→` はゲーム内で「戻る」以上のことをしない。

> **補足**: 左ボタンでカーソルが右に進むので、物理配置と移動方向は逆になる。
> 3 ボタンで選択・決定・キャンセルを賄うには、選択ボタンを一方向の送りにするのが素直。
> 違和感が強い場合は、
> 「`←` `→` で双方向移動、キャンセルは中央長押し」に変更する余地がある。
> ただし長押し判定が必要になるため、まずは循環方式で試す。

---

## 2. 画面レイアウト

縦 128px を 3 段に分ける。

```
        0 ┌──────────────────────────────┐
          │  ステータスバー (16px)        │   世代・日数・ごきげん・空腹
       16 ├──────────────────────────────┤
          │                              │
          │                              │
          │   キャラクター領域 (96px)     │   キャラ・うんち・アニメーション
          │                              │
          │                              │
      112 ├──────────────────────────────┤
          │  メニューバー (16px)          │   アイコン6個、選択中は反転
      128 └──────────────────────────────┘
```

96 × 96 のキャラ領域は 9216 px。ドット絵に十分な余裕があり、
表情や動きで状態を表現できる。

---

## 3. 各画面

### 3.1 メイン画面（アイドル）

常態。キャラが歩き回り、うんちが溜まる。

```
┌──────────────────────────────┐
│ G3  D4      ♥♥♡♡   ▮▮▮▯      │
├──────────────────────────────┤
│                              │
│           ( ˘ ω ˘ )          │
│            /|   |\           │
│           ~        💩        │
│                              │
├──────────────────────────────┤
│ [🍚] 🎮  🧹  💊  📖  ⚙       │
└──────────────────────────────┘
```

- `G3` = 第3世代、`D4` = ゲーム内4日目
- `♥` = ごきげん（4段階）、`▮` = 満腹度（4段階）
- 選択中のアイコンは白黒反転

| ボタン | 動作 |
|---|---|
| `←` 選択 | カーソルを1つ右へ（循環） |
| `🔥` 決定 | 選択中のメニューを実行 |
| `→` キャンセル | カーソル選択を解除（何も選んでいない状態に戻る） |

### 3.2 メニュー項目

| アイコン | 名前 | 動作 |
|---|---|---|
| 🍚 | ごはん | サブメニュー（ごはん / おやつ）へ |
| 🎮 | あそぶ | ミニゲームへ |
| 🧹 | そうじ | うんちを消す。即実行 |
| 💊 | くすり | 病気を治す。病気でなければ「げんき！」表示 |
| 📖 | しつけ | 呼び出し中なら叱る。そうでなければ無反応（＝ミス扱いにしない） |
| ⚙ | メニュー | サブメニュー（ステータス / セーブ / おわる）へ |

### 3.3 ごはんサブメニュー

```
┌──────────────────────────────┐
│ G3  D4      ♥♥♡♡   ▮▮▮▯      │
├──────────────────────────────┤
│                              │
│      ┌────────────────┐      │
│      │ ▶ ごはん        │      │
│      │   おやつ        │      │
│      └────────────────┘      │
│                              │
├──────────────────────────────┤
│  ← えらぶ  🔥 きめる  → もどる │
└──────────────────────────────┘
```

- ごはん = 満腹度 +1、体重 +1
- おやつ = ごきげん +1、体重 +2

| ボタン | 動作 |
|---|---|
| `←` 選択 | 項目を1つ下へ（循環） |
| `🔥` 決定 | 実行 → アニメーション → メイン画面 |
| `→` キャンセル | メイン画面へ戻る |

### 3.4 ⚙ サブメニュー

「メインメニューに戻る」と「セーブ」はここに置く。

```
┌──────────────────────────────┐
│ G3  D4      ♥♥♡♡   ▮▮▮▯      │
├──────────────────────────────┤
│      ┌────────────────┐      │
│      │ ▶ ステータス    │      │
│      │   セーブ        │      │
│      │   おわる        │      │
│      └────────────────┘      │
│                              │
├──────────────────────────────┤
│  ← えらぶ  🔥 きめる  → もどる │
└──────────────────────────────┘
```

| 項目 | 動作 |
|---|---|
| ステータス | ステータス画面へ |
| セーブ | 即セーブし、「セーブしました」を数秒表示してメイン画面へ |
| おわる | **セーブしてから** Xous のメインメニューへ復帰 |

**設計意図**: セーブは自動（状態変化時＋ディープスリープ突入前）で行われるため、
本来この項目は不要。しかし USB を抜く運用なので、
「抜く前に自分で保存できる」という安心感のために明示的に置く。
`おわる` も必ずセーブを挟むので、実質どちらを選んでも安全に終われる。

> **代替案**: 公式アプリ（`dc34-vault`）では `∴`（ジョグ押し込み）が
> 「メニューを開く」システム的な役割を持っている（`main.rs:395`）。
> 3ボタン縛りを緩めてよいなら、`∴` に「おわる」を割り当てる方が
> 他アプリとの一貫性は高い。今回はメニュー項目として実装する。

### 3.5 ミニゲーム（左右あて）

キャラがどちらを向くかを当てる。

```
┌──────────────────────────────┐
│         あそぶ  2/5          │
├──────────────────────────────┤
│                              │
│         ( ˘ ω ˘ )            │
│                              │
│      ▶ ひだり    みぎ        │
│                              │
├──────────────────────────────┤
│  ← えらぶ  🔥 きめる  → やめる │
└──────────────────────────────┘
```

- 5 回勝負。3 回以上当てるとごきげん +2、体重 -1
- 負け越したらごきげん +1、体重 -1

| ボタン | 動作 |
|---|---|
| `←` | **ひだり** を選ぶ |
| `🔥` 決定 | answer を確定 |
| `→` | **みぎ** を選ぶ |

**この画面だけ `←` `→` の意味が変わる**。左右を指すことがゲームそのものなので、
選択・キャンセルではなく回答そのものに割り当てる。
中断はできないが、5回で終わるので実害は無い。

### 3.6 ステータス画面

```
┌──────────────────────────────┐
│          ステータス           │
├──────────────────────────────┤
│  なまえ   ハムりん           │
│  せだい   3                  │
│  ねんれい 4にち              │
│  たいじゅう 25g              │
│  しつけ   ▮▮▮▯               │
│  ケアミス 2かい              │
├──────────────────────────────┤
│         → でもどる           │
└──────────────────────────────┘
```

| ボタン | 動作 |
|---|---|
| `→` キャンセル / `🔥` 決定 | メイン画面へ戻る |

### 3.7 睡眠中

ゲーム内 0〜4 時（累計稼働 20 分）。

```
┌──────────────────────────────┐
│ G3  D4                       │
├──────────────────────────────┤
│                              │
│              z z             │
│         ( ─ ω ─ )            │
│                              │
│                              │
├──────────────────────────────┤
│         おやすみ…            │
└──────────────────────────────┘
```

- **操作を受け付けない**（メニューバーも消す）
- 「就寝中に消灯しないとケアミス」という判定は**採用しない**。
  絶対時刻を持たず、電源が落ちている間は進行も止まる設計なので、
  プレイヤーが消灯できたかどうかを公平に判定できない

### 3.8 呼び出し（アラート）

空腹・ごきげんが 0 になった、または病気になったとき。

```
┌──────────────────────────────┐
│ G3  D4      ♡♡♡♡   ▯▯▯▯      │
├──────────────────────────────┤
│          !    !              │
│                              │
│         ( >  <  )            │
│          /|   |\             │
│                              │
├──────────────────────────────┤
│ [🍚] 🎮  🧹  💊  📖  ⚙       │
└──────────────────────────────┘
```

- メイン画面のバリエーション。キャラが困り顔になり、`!` が点滅する
- **25 分以内**に対応しないとケアミス 1 回
- 音も LED も無いので、**気づけるのは画面を見たときだけ**。
  USB 給電で机上に置く運用のため、仕様として受け入れる

### 3.9 進化

```
┌──────────────────────────────┐
│                              │
│         おおきくなった！      │
│                              │
│         ( ^ ω ^ )            │
│          /|   |\             │
│                              │
│                              │
├──────────────────────────────┤
│        🔥 でつづける          │
└──────────────────────────────┘
```

### 3.10 寿命 → 世代交代

累計稼働 24 時間を生き切った場合。

```
┌──────────────────────────────┐
│                              │
│  じゅみょうをまっとうしました  │
│                              │
│        ( - ω - )  ✧          │
│                              │
│    せだい 3 → 4              │
│    とくせい をひきついだ      │
│                              │
├──────────────────────────────┤
│        🔥 でつづける          │
└──────────────────────────────┘
```

`🔥` で **卵画面へ**。世代番号 +1、育て方に応じた特性を引き継ぐ。

### 3.11 早死に → ゲームオーバー

ケアミスの累積で死んだ場合。

```
┌──────────────────────────────┐
│                              │
│                              │
│         ( x  x )             │
│                              │
│      おわってしまった…       │
│                              │
│    せだい 3 → 1              │
│                              │
├──────────────────────────────┤
│      🔥 ではじめから          │
└──────────────────────────────┘
```

`🔥` で **卵画面へ**。世代番号はリセット、引き継ぎ無し。

### 3.12 卵（開始）

```
┌──────────────────────────────┐
│ G1  D0                       │
├──────────────────────────────┤
│                              │
│            ___               │
│           /   \              │
│          |  ~  |             │
│           \___/              │
│                              │
├──────────────────────────────┤
│      うまれるまで…           │
└──────────────────────────────┘
```

累計稼働 10 分で孵化 → 赤ちゃんへ。この間もメニューバーは出さない。

---

## 4. 画面遷移

```mermaid
stateDiagram-v2
    [*] --> Egg: 初回起動 / 世代交代
    Egg --> Main: 孵化（累計10分）

    Main --> FeedMenu: 🍚
    FeedMenu --> Main: 🔥 決定 / → キャンセル

    Main --> MiniGame: 🎮
    MiniGame --> Main: 5回終了 / → 中断

    Main --> Main: 🧹 そうじ（即実行）
    Main --> Main: 💊 くすり（即実行）
    Main --> Main: 📖 しつけ（即実行）

    Main --> SysMenu: ⚙
    SysMenu --> Status: ステータス
    SysMenu --> Main: セーブ（保存して戻る）
    SysMenu --> [*]: おわる（保存して Xous メニューへ）
    SysMenu --> Main: → キャンセル
    Status --> Main: → / 🔥

    Main --> Alert: メーター0 / 病気
    Alert --> Main: 対応した
    Alert --> Main: 25分経過（ケアミス+1）

    Main --> Sleep: ゲーム内 0時
    Sleep --> Main: ゲーム内 4時

    Main --> Evolve: ステージ到達
    Evolve --> Main: 🔥

    Main --> Farewell: 累計24時間到達
    Farewell --> Egg: 🔥（世代+1、引き継ぎあり）

    Main --> GameOver: ケアミス累積
    GameOver --> Egg: 🔥（世代リセット）
```

---

## 5. ボタン割り当て一覧

| 画面 | `←` 選択 | `🔥` 決定 | `→` キャンセル | `∴` |
|---|---|---|---|---|
| メイン | カーソル送り | 実行 | 選択解除 | ゲームを抜ける |
| ごはんメニュー | 項目送り | 実行 | メインへ戻る | ゲームを抜ける |
| ⚙ メニュー | 項目送り | 実行 | メインへ戻る | ゲームを抜ける |
| ミニゲーム | **ひだり** | 確定 | **みぎ** | ゲームを抜ける |
| ステータス | — | 戻る | 戻る | ゲームを抜ける |
| 睡眠中 | — | — | — | ゲームを抜ける |
| 進化 / 寿命 / 終了 | — | つづける | — | ゲームを抜ける |
| 卵 | — | — | — | ゲームを抜ける |

**原則**: `🔥` は常に「進む・決定」、`→` は常に「戻る」、`←` は常に「選ぶ」、
`∴` は常に「ゲームを抜ける」。ミニゲームだけが `←` `→` の例外。`↑` `↓` は未使用。

睡眠中も `∴` は受け付ける。抜けられなくなる方が実害が大きい。

---

## 6. 実装メモ

- ステータスバーとメニューバーは共通コンポーネントにして、全画面で使い回す
- キャラの表情は「通常 / 困り / 喜び / 病気 / 睡眠 / 死亡」の 6 種類あれば足りる。
  1 枚 96×96 の 1bpp ビットマップで 1152 バイト、6 枚で約 7 KB
- 画面の再描画は状態変化時のみ。`dc34-vault` の `redraw()` が
  「前回のモードと違うときだけ描く」構造になっているので、それに倣う
- ミニゲームの乱数は TRNG が使える（`bao1x-emu` に hosted 用シムもある）
- `↑` `↓` `∴` は受け取っても無視する。
  ただし `∴` は公式アプリでメニューキーとして使われているため、
  将来的に「おわる」を割り当てる余地は残しておく
